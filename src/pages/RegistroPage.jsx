...
export default function RegistroPage() {
  ...
  const inputStyle = (valid) => ({
    ...
    border: `1px solid ${valid === false ? '#EF4444' : valid === true ? '#10B981' : 'var(--border)'}`,
  ...
  if (paso === 3) return (
    <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ textAlign: 'center', maxWidth: 400 }}>
        ...
        <h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8, color: 'var(--text)' }}>¡Cuenta creada!</h2>
  ...
  return (
    <div style={{ minHeight: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--bg-soft)' }}>
      <div style={{ width: '100%', maxWidth: 480 }}>

        {/* Logo */}
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <img src="/logo.png" alt="RedNorte" style={{ height: 56 }} />
          <h1 style={{ fontSize: 22, fontWeight: 800, marginTop: 12, color: 'var(--text)' }}>
            ...
            <React.Fragment key={p}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                <div style={{
                  ...
                  background: i < paso ? '#10B981' : i === paso ? '#2563EB' : 'var(--border)',
                  color: i <= paso ? 'white' : 'var(--text-muted)',
                }}>{i < paso ? '✓' : i + 1}</div>
                <div style={{ fontSize: 11, marginTop: 4, color: i === paso ? '#2563EB' : 'var(--text-muted)', fontWeight: i === paso ? 600 : 400 }}>{p}</div>
              </div>
              {i < PASOS.length - 1 && <div style={{ flex: 1, height: 2, background: i < paso ? '#10B981' : 'var(--border)', marginBottom: 16 }} />}
            </React.Fragment>
          ))}
        </div>

        <div style={{ background: 'var(--card)', borderRadius: 16, padding: '2rem', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          ...
          {paso === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 4, color: 'var(--text)' }}>Datos de acceso</h3>
                ...
                {form.pass && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                      {[1,2,3,4].map(i => (
                        <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i <= fuerza ? fuerzaColor : 'var(--border)', transition: 'all 0.3s' }} />
                      ))}
                    </div>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>CONFIRMAR CONTRASEÑA</label>
          ...
          {paso === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 4, color: 'var(--text)' }}>Datos personales</h3>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>PRIMER APELLIDO *</label>
                  <input style={inputStyle(form.apellido1 ? true : undefined)} value={form.apellido1} onChange={set('apellido1')} placeholder="González" />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>SEGUNDO APELLIDO</label>
                  <input style={inputStyle()} value={form.apellido2} onChange={set('apellido2')} placeholder="Muñoz" />
                </div>
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>RUT *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>FECHA DE NACIMIENTO *</label>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>SEXO *</label>
          ...
          {paso === 2 && (
            <div>
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: 'var(--text)' }}>Confirma tus datos</h3>
              {[
                ['Correo', form.mail],
                ['Contraseña', '••••••••'],
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
            {paso > 0 && (
              <button onClick={() => { setPaso(paso - 1); setError(''); }} style={{
                ...
                background: 'var(--bg-soft)', cursor: 'pointer', fontSize: 14, fontWeight: 500,
                color: 'var(--text)',
              }}>← Volver</button>
            )}
