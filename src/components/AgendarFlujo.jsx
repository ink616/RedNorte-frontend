...
import { listarBloquesDisponibles, reservarBloque, crearConsulta, listarEstablecimientos, enviarConfirmacionCita } from '../service/api';
...
/**
 * Flujo completo de agendamiento (fecha -> horario -> confirmar),
 * extraido de AgendarPage para poder reutilizarlo tanto en la pagina
 * de pantalla completa /agendar como en el modal que SaludBot abre
 * desde el chat (AgendarCitaModal.jsx). La logica de negocio (llamadas
 * a la API, validaciones) es exactamente la misma en ambos lugares;
 * lo unico que cambia es el contenedor visual que lo envuelve.
 *
 * onExito({ fecha, hora, especialidad, doctor, lugar }): se dispara una
 * sola vez, justo cuando la reserva se confirma con exito (no espera a
 * que el usuario cierre nada). En la pagina nadie lo escucha; en el
 * modal del chat, SaludBot lo usa para anunciar la cita en la
 * conversacion sin cerrar el modal (el paciente decide cuando cerrarlo,
 * via "Agendar otra" o el boton de cerrar).
 *
 * onCancelar / onVerConsultas: por defecto navegan a /mis-consultas
 * (comportamiento original de la pagina completa). El modal los
 * sobreescribe para cerrarse a si mismo en vez de abandonar el chat.
 */
export default function AgendarFlujo({ onExito, onCancelar, onVerConsultas }) {
  ...
  const handleCancelar = () => { if (onCancelar) onCancelar(); else navigate('/mis-consultas'); };
  const handleVerConsultas = () => { if (onVerConsultas) onVerConsultas(); else navigate('/mis-consultas'); };
  ...
  const handleConfirmar = async () => {
    ...
    try {
      ...
      // Fire-and-forget: el correo de confirmacion nunca debe bloquear ni
      // revertir la pantalla de exito. La cita ya quedo agendada de verdad
      // en el paso anterior; si el correo falla (SMTP caido, etc.) el
      // paciente igual ve su cita confirmada, solo no le llega el mail.
      enviarConfirmacionCita(
        usuario?.mail,
        nombrePaciente,
        doc?.especialidad || 'General',
        fecha,
        formatHora(bloqueSeleccionado.horaInicio),
      ).catch(() => {});

      onExito?.({
        fecha,
        hora: formatHora(bloqueSeleccionado.horaInicio),
        especialidad: doc?.especialidad || 'General',
        doctor: doc?.nombre || '',
        lugar: nombreEst(bloqueSeleccionado.establecimientoId),
      });
    } catch {
    ...
    return (
      <div className="ag-exito-wrap">
        ...
        <div className="ag-exito-btns">
          ...
          <button className="btn btn-primary btn-lg" onClick={handleVerConsultas}>
            Ver mis consultas
          </button>
        </div>
      </div>
        ...
        return (
          <div className="ag-card">
            ...
            <div className="ag-confirmar-btns">
              <button className="btn btn-ghost btn-lg" onClick={handleCancelar}>Cancelar</button>
