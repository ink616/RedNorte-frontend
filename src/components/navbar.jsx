...
export default function Navbar() {
  ...
  const linkStyle = (path, exact = false) => {
    ...
    return {
      padding: '7px 14px', borderRadius: 20, fontSize: 13.5,
  ...
  const ADMIN_LINKS = [
    ...
    { to: '/admin/establecimientos', label: 'Establecimientos', icon: '🏥' },
    { to: '/admin/agenda',           label: 'Agenda',           icon: '📅' },
    { to: '/admin/reasignacion',     label: 'Reasignación',     icon: '🔄' },
  ];
  ...
  return (
    <nav style={{
      ...
      <Link to="/" style={{ display:'flex', alignItems:'center', gap:10, textDecoration:'none', flexShrink:0 }}>
        <img src="/logo.png" alt="RedNorte" style={{ height:50, width:'auto' }} />
        <div style={{ lineHeight:1.2 }}>
          <div style={{ color:'white', fontWeight:800, fontSize:20, letterSpacing:'-0.3px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:2 }}>
        ...
        {esAdmin && (
          <div style={{ display:'flex', alignItems:'center', gap:2 }}>
            {ADMIN_LINKS.map(a => (
              <Link key={a.to} to={a.to} style={linkStyle(a.to)} title={a.label}>
